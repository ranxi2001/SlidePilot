...
- `npm run test:api` - API route tests, including NDJSON progress streaming in mock mode.
...
- Mock pipeline tests still use Playwright for screenshot QA and PDF export, so Chromium must be installed with `npx playwright install chromium`.
